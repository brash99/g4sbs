...
extern PolNucleonRotate* gPolRot;
...
G4HadronicProcess::FillResult(G4HadFinalState * aR, const G4Track & aT)
{
  ...
  if( gPolRot ){
    if( IsPolMaterial(aT) ){     // does tracking material contain H,C?
...
//-----------------------------------------------------------------------------
inline G4bool G4HadronicProcess::IsPolMaterial( const G4Track & aT )
{
  // Determine if the material currently tracking through
  // contains H or C or Cu
  //
  //return true;
  ...
  for( G4int i=0; i<nelm; i++ ){
    ...
    if( (z == 1) || (z == 6) || (z == 26) || (z == 29) ) { return true; }
  }
  return false;
}

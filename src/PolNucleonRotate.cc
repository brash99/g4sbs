...
//   if( Pt == 1 ) {
//     G4cout << "Ay = " << gAy << "\t channel = " << gChannel << G4endl;
//     G4cout << "pt = " << Pt << G4endl;
//   }
//   else
//     G4cout << "unpol" << G4endl;
...
//----------------------------------------------------------------------------
G4double PolNucleonRotate::AyCE(G4double pt)
{  
  ...
  // NB t is -ve
  //
  /* original from free elastic charge exchange
  if(t > ftmin) return 0.0;
  else if( t > -0.4 ) return 1.3*t;
  else if( t < -2.0 ) return 0.0;
  else return -0.52;
  */
  // From fit to Dubna n+Cu -> p + X data...good up to pt = 0.85 GeV/c
  pt = pt/1000.;   // GeV/c
  if(pt > 0.85)
    pt = 0.85;
  G4double ay = 0.01 * (97.7733*pt - 442.959*pt*pt + 1064.6*pt*pt*pt -
			1188.65*pt*pt*pt*pt + 484.353*pt*pt*pt*pt*pt);
  return ay;
}
